...
TAIL1=`ls 02a_mapped_museum/*bam | awk -F "/" '{print $NF}' | awk '{print substr($0,14)}' | head -n 1`
TAIL2="RG.bam"
...
#Create input file
ls $INPUT/*bam | awk -F "/" '{print $NF}' | awk -F "_" '{print $1}' > mus.names 
...
echo "while read NAME1 <&1; do java -jar $PICARD AddOrReplaceReadGroups \
       ...
       RGPL=ILLUMINA \
       RGPU=unit1 \
       RGSM=$NAME1; done 1<mus.names" >> 02b.0_AddRG.log
...
CREATE_INDEX=true; done 2<mus.names" >> 02b.ProcessBams.log
...
REMOVE_DUPLICATES=true \
VALIDATION_STRINGENCY=SILENT \
CREATE_INDEX=true; done 2<mus.names
...
-I $INPUT/$NAME3.rmdup.bam; done 3<mus.names
...
-o $OUTPUT/$NAME3.realn.bam; done 3<mus.names
