#!/bin/bash
#$ -S /bin/bash
#$ -N E3.MODC.processbams  ##job name
#$ -l tmem=32G #RAM
...
TAIL1=`ls 02a_mapped_modern/*bam | awk -F "/" '{print $NF}' | awk '{print substr($0,14)}' | head -n 1`
TAIL2="RG.bam"
...
echo "while read NAME1 <&1; do java -jar $PICARD AddOrReplaceReadGroups \
       ...
       RGPL=ILLUMINA \
       RGPU=unit1 \
       RGSM=$NAME1; done 1<modc.names" >> 02b.0_AddRG.log
...
REMOVE_DUPLICATES=true \
VALIDATION_STRINGENCY=SILENT \
CREATE_INDEX=true; done 2<modc.names
...
-I $INPUT/$NAME3.rmdup.bam; done 3<modc.names
...
-o $OUTPUT/$NAME3.realn.bam; done 3<modc.names
